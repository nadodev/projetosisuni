    ...
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex gap-2">
                ...
                <button
                    onclick="copyFormLink('{{ route('forms.show', $anamnese->uuid) }}')"
                    ...
                    @foreach($anamnese->form->fields as $field)
                        <div class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }} px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                                {{ $anamnese->respostas[$field->id] ?? 'Não informado' }}
                            </dd>
                        </div>
                    @endforeach
                </dl>
                        ...
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                <p class="text-sm font-medium text-gray-900">
                                    {{ $evolucao->data_evolucao ? $evolucao->data_evolucao->format('d/m/Y') : '' }} às {{ $evolucao->hora_evolucao ? $evolucao->hora_evolucao->format('H:i') : '' }}
                                </p>
