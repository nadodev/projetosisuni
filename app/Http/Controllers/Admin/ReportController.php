...
class ReportController extends Controller
{
    ...
    public function exportTurmasPDF()
    {
        ...
        $pdf = PDF::loadView('admin.reports.pdf.turmas', ['turmas' => $turmas]);
    ...
    public function exportCategoriasPDF()
    {
        ...
        $pdf = PDF::loadView('admin.reports.pdf.categorias', ['categorias' => $categorias]);
    ...
    public function exportEstudantesPDF()
    {
        ...
        $pdf = PDF::loadView('admin.reports.pdf.estudantes', ['estudantes' => $estudantes]);
    ...
    public function exportProfessoresPDF()
    {
        ...
        $pdf = PDF::loadView('admin.reports.pdf.professores', ['professores' => $professores]);
    ...
    public function exportUsuariosPorCategoriaPDF()
    {
        ...
        $pdf = PDF::loadView('admin.reports.pdf.usuarios-por-categoria', ['usuarios' => $usuarios]);
        return $pdf->download('relatorio-usuarios-por-categoria.pdf');
    }

    public function dashboard()
    {
        $totalUsers = User::where('role', '!=', 'user_admin')
            ->where('institution_id', auth()->user()->institution_id)
            ->count();

        $totalTeachers = User::where('role', 'user_teacher')
            ->where('institution_id', auth()->user()->institution_id)
            ->count();
        ...
        $totalStudents = User::where('role', 'user_student')
            ->where('institution_id', auth()->user()->institution_id)
            ->count();

        $totalClasses = Turma::where('institution_id', auth()->user()->institution_id)
            ->count();

        $recentStudents = User::where('role', 'user_student')
            ->where('institution_id', auth()->user()->institution_id)
            ->latest()
            ->take(5)
            ->get();

        $recentTeachers = User::where('role', 'user_teacher')
            ->where('institution_id', auth()->user()->institution_id)
            ->latest()
            ->take(5)
            ->get();
        ...
        return view('admin.reports.dashboard', compact('totalUsers', 'totalTeachers', 'totalStudents', 'totalClasses', 'recentStudents', 'recentTeachers', 'usersByRole', 'usersByStatus'));
    }
}
